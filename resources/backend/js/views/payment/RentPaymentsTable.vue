<template>
    ...
    <div class="box box-widget m-margin-tb">
    	<div class="box-body">
            ...
            :headers="['Type', 'Duration', 'Due Date', 'Amount', 'Notes', 'Status']"
            :columns="['type_id', 'name', 'duration', 'due_date', 'amount', 'note', 'status']"
    		:filters="filters"
    		...
    		>
    			...
    			<tbody slot="body">
                    <template v-for="item in items">
                        <tr v-for="bill in item.rentBills">
                            <td>Rent</td>
                            <td>{{ item.duration }}</td>
                            <td>{{ bill.date }}</td>
                            <td>{{ bill.invoice.amount }}</td>
                            <td>
                                <p style="white-space: nowrap; width: 50px; overflow: hidden; text-overflow: ellipsis;">
                                    {{item.note}}
                                </p>
                            </td>
                            <td v-if="bill.invoice.status !== 1"><span :class="bill.stylelabel" style="border-radius: 4px;">{{ bill.label }}</span></td>
                            <td v-if="bill.invoice.status === 1"><span class="btn-success btn-flat btn-xs" style="border-radius: 4px;">PAID</span></td>
                            <td v-if="bill.invoice.status !== 1">
                                <a :href="bill.actions.paymenturl" class="btn btn-primary btn-flat btn-sm">Pay</a>
                            </td>
                            <td v-if="bill.invoice.status === 1">
                                <a :href="bill.actions.print" class="btn btn-primary btn-flat btn-sm">Print Invoice</a>
                            </td>
                        </tr>
                    </template>
    			</tbody>
